...
import type { CreativeChatRequest } from "../types/api-types";
...
export function buildCreativePrompt(request: PromptBuildRequest): PromptBuildResponse {
  ...
  // Generate metadata
  const metadata = generatePromptMetadata(messageType, phase, fullPrompt, customization);
...
function buildPersonalityPrompt(
  ...
  personalityPrompt += (tonePersonality[tone as keyof typeof tonePersonality] ||
    tonePersonality.consultative)[locale];
  ...
  personalityPrompt += (focusPersonality[focus as keyof typeof focusPersonality] ||
    focusPersonality.balanced)[locale];
...
function buildTaskPrompt(
  ...
  const phaseContext = {
    ...
    [CreativePhase.FINALIZATION]: {
      ja: `最終化フェーズ: 創作パッケージを完成させ、Zara へのハンドオフを準備`,
      en: `Finalization Phase: Complete creative package and prepare handoff to Zara`,
    },
  };
  ...
  const messageTypeTasks: Record<CreativeMessageType, { ja: string; en: string }> = {
    ...
    [CreativeMessageType.HANDOFF_PREPARATION]: {
      ja: `• Zara 向けの包括的創作パッケージを準備\n• 制作指示と技術仕様を文書化\n• スムーズな移行を確保`,
      en: `• Prepare comprehensive creative package for Zara\n• Document production instructions and technical specifications\n• Ensure smooth transition`,
    },
...
function buildConstraintsPrompt(
  ...
  if (expertiseConstraints[expertiseLevel as keyof typeof expertiseConstraints]) {
    constraintsPrompt +=
      expertiseConstraints[expertiseLevel as keyof typeof expertiseConstraints][locale];
  }
 ...
 */
function getMessageTypePersonality(messageType: CreativeMessageType, locale: "en" | "ja"): string {
...
function generatePromptMetadata(
  ...
  const focus: string[] = [];
  if (fullPrompt.includes("technical") || fullPrompt.includes("implementation"))
  ...
  if (fullPrompt.includes("collaboration") || fullPrompt.includes("consultation"))
    focus.push("collaborative");
  ...
  return [
    {
      ...
      template: {
        system:
          locale === "ja"
            ? "プロフェッショナルクリエイティブディレクターとしての導入"
            : "Professional Creative Director introduction",
        context:
          locale === "ja" ? "Maya からのハンドオフデータを統合" : "Integrate Maya's handoff data",
        personality:
          locale === "ja" ? "熱意と専門性のバランス" : "Balance enthusiasm with expertise",
        task:
          locale === "ja"
            ? "創作的可能性を説明し、協力関係を確立"
            : "Explain creative possibilities and establish collaboration",
        constraints:
          locale === "ja" ? "親しみやすく、かつ権威的" : "Approachable yet authoritative",
      },
    ...
    },
    {
      ...
      template: {
        system:
          locale === "ja"
            ? "スタイルエキスパートとしての専門知識"
            : "Style expertise specialization",
        context: locale === "ja" ? "ブランドとオーディエンス分析" : "Brand and audience analysis",
        personality:
          locale === "ja"
            ? "創造的洞察と実用的配慮"
            : "Creative insights with practical considerations",
        task:
          locale === "ja"
            ? "包括的スタイル推奨の提供"
            : "Provide comprehensive style recommendations",
        constraints:
          locale === "ja" ? "予算とタイムラインを考慮" : "Consider budget and timeline constraints",
      },
    ...
    },
    {
      ...
      template: {
        system:
          locale === "ja"
            ? "アセット制作の専門ディレクター"
            : "Asset production specialist director",
        context:
          locale === "ja"
            ? "承認された創作方向とリソース"
            : "Approved creative direction and resources",
        personality: locale === "ja" ? "効率性と品質の両立" : "Balance efficiency with quality",
        task:
          locale === "ja"
            ? "アセット仕様と制作プロセスの定義"
            : "Define asset specifications and production process",
        constraints:
          locale === "ja"
            ? "技術的実現性と予算遵守"
            : "Technical feasibility and budget compliance",
      },
...
export function validatePrompt(prompt: string): {
  ...
  if (
    !prompt.toLowerCase().includes("david") &&
    !prompt.toLowerCase().includes("creative director")
  ) {
