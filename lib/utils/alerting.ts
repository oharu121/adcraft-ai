...
import { SecurityMonitorService } from "../monitor/security-monitor";
...
export class AlertingService {
  ...
  public addRule(rule: Omit<AlertRule, "createdAt" | "updatedAt">): string {
    ...
    this.logger.info(
      `Alert rule ${rule.enabled ? "added" : "added (disabled)"}: ${rule.name}`,
    ...
    const defaultRules: Array<Omit<AlertRule, "createdAt" | "updatedAt">> = [
      {
        ...
        enabled: true,
        source: "performance",
        conditions: [{ metric: "api_error_rate", aggregation: "avg", value: 0 }],
      ...
      },
      {
        ...
        enabled: true,
        source: "performance",
        conditions: [{ metric: "api_response_time", aggregation: "avg", value: 0 }],
      ...
      },
      {
        ...
        enabled: true,
        source: "cost",
        conditions: [{ metric: "budget_usage_percent", value: 0 }],
      ...
      },
      {
        ...
        enabled: true,
        source: "security",
        conditions: [{ metric: "security_events", field: "severity", value: "critical" }],
      ...
      },
      {
        ...
        enabled: true,
        source: "performance",
        conditions: [{ metric: "memory_usage_percent", value: 0 }],
  ...
  private async getCurrentValue(rule: AlertRule): Promise<number | null> {
    try {
      ...
      this.logger.error(
        `Failed to get current value for rule: ${rule.name}`,
  ...
  private async createAlert(rule: AlertRule, currentValue: number): Promise<void> {
    ...
    const totalActiveAlerts = Array.from(this.alerts.values()).filter(
      (alert) => alert.status === "active"
    ).length;

    if (totalActiveAlerts >= this.config.maxTotalAlerts) {
      this.logger.warn(
        "Maximum total alerts reached, skipping new alert",
    ...
    switch (channel.type) {
      ...
      default:
        this.logger.warn(`Unsupported notification channel: ${channel.type}`, {
          service: "alerting",
          operation: "send_channel_notification",
        });
    }
  }
  ...
  public healthCheck(): boolean {
    try {
      ...
      this.logger.error(
        "Alerting service health check failed",
