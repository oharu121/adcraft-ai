...
import { SessionStatus, AgentType, HandoffStatus, TopicStatus } from "@/lib/agents/product-intelligence/enums";
...
export class ProductIntelligenceFirestore {
      ...
      const firestoreDoc: ProductIntelligenceSessionDoc = {
        ...
        user: {
          ...
          joinedAt: Timestamp.fromDate(
            sessionData.user.joinedAt ? new Date(sessionData.user.joinedAt) : now
          ),
          lastActivity: Timestamp.fromDate(
            sessionData.user.lastActivity ? new Date(sessionData.user.lastActivity) : now
          ),
        },
        product: {
          ...sessionData.product,
          uploadTimestamp: Timestamp.fromDate(
            sessionData.product.uploadTimestamp
              ? new Date(sessionData.product.uploadTimestamp)
              : now
          ),
        },
        conversation: {
          ...
          lastMessageTimestamp: Timestamp.fromDate(
            sessionData.conversation.lastMessageTimestamp
              ? new Date(sessionData.conversation.lastMessageTimestamp)
              : now
          ),
        },
        performance: {
          ...
          startTime: Timestamp.fromDate(
            sessionData.performance.startTime ? new Date(sessionData.performance.startTime) : now
          ),
          lastActivity: Timestamp.fromDate(
            sessionData.performance.lastActivity
              ? new Date(sessionData.performance.lastActivity)
              : now
          ),
        },
        handoff: {
          ...sessionData.handoff,
          handoffTimestamp: sessionData.handoff.handoffTimestamp
            ? Timestamp.fromDate(new Date(sessionData.handoff.handoffTimestamp))
            : undefined,
        },
        rateLimit: {
          ...sessionData.rateLimit,
          lastRequestTime: Timestamp.fromDate(
            sessionData.rateLimit.lastRequestTime
              ? new Date(sessionData.rateLimit.lastRequestTime)
              : now
          ),
          resetTime: Timestamp.fromDate(
            sessionData.rateLimit.resetTime
              ? new Date(sessionData.rateLimit.resetTime)
        ...
        health: {
          ...
          lastHeartbeat: Timestamp.fromDate(
            sessionData.health.lastHeartbeat ? new Date(sessionData.health.lastHeartbeat) : now
          ),
        },
        metadata: {
    ...
    try {
      ...
      if (this.isMock()) {
        ...
        if (existingSession) {
          ...
          const updatedDoc = {
            ...existingSession,
            ...this.convertSessionStateToFirestoreDoc(updates as SessionState),
          };
   ...
   */
  private convertSessionStateToFirestoreDoc(
    session: SessionState
  ): Partial<ProductIntelligenceSessionDoc> {
    ...
    if (session.conversation?.lastMessageTimestamp) {
      doc.conversation.lastMessageTimestamp = Timestamp.fromDate(
        new Date(session.conversation.lastMessageTimestamp)
      );
    }
    ...
    if (session.rateLimit?.lastRequestTime) {
      doc.rateLimit.lastRequestTime = Timestamp.fromDate(
        new Date(session.rateLimit.lastRequestTime)
      );
    }
