...
export class PromptRefiner {
  ...
  public async refinePrompt(prompt: string, sessionId?: string): Promise<PromptRefinement> {
    try {
      ...
      const costCheck = await this.costTracker.canProceedWithOperation(0.20);
...
Keep responses concise but informative. Focus on actionable advice.`;
  ...
  public analyzePrompt(prompt: string): {
    ...
    const visualKeywords = ['camera', 'lighting', 'color', 'visual', 'shot', 'angle', 'composition'];
    ...
    const motionKeywords = ['movement', 'motion', 'moving', 'flowing', 'walking', 'running', 'dancing'];
    const hasMotion = motionKeywords.some(keyword => 
      prompt.toLowerCase().includes(keyword)
    );
    ...
    const styleKeywords = ['cinematic', 'dramatic', 'peaceful', 'energetic', 'artistic', 'realistic'];
