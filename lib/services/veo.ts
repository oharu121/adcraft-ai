...
export class VeoService {
   ...
   */
  private async generateVideoReal(
    ...
    // Prepare request for Veo API
    // Try using inline_data format based on Gemini API documentation
    const instance: any = {
   ...
   */
  private async generateVideoMock(
    request: VideoGenerationRequest
  ): Promise<VideoGenerationResponse> {
  ...
  private async getJobStatusReal(operationName: string): Promise<VideoJobStatus> {
    ...
    if (data.error) {
      ...
      if (data.error.code === "INTERNAL") {
        userFriendlyError =
          "Veo service temporarily unavailable. This is a known issue with Google's video generation service. Please try again in a few minutes.";
        ...
        userFriendlyError =
          "Video request format issue. Please try with a shorter or simpler prompt.";
      } else if (
        data.error.message?.includes("timeout") ||
        data.error.message?.includes("deadline")
      ) {
    ...
    console.log(
      `🔍 data.response?.generateVideoResponse exists: ${!!data.response?.generateVideoResponse}`
    );
    ...
    if (videoResponse && videoResponse.raiMediaFilteredCount > 0) {
      const filterReason =
        videoResponse.raiMediaFilteredReasons?.[0] || "Content filtered by safety policies";
      ...
      if (filterReason.includes("audio")) {
        userFriendlyError =
          "Video generation failed due to audio content restrictions. This often occurs with non-English text or complex narration. Try simplifying the product description or using English text.";
      }
    ...
    console.log(`🔍 Expected path not found. Full response structure:`, {
      ...
      generateVideoResponseKeys: data.response?.generateVideoResponse
        ? Object.keys(data.response.generateVideoResponse)
        : "none",
    });
    ...
    try {
      ...
      if (elapsed < 3000) {
        ...
        const progress = Math.min(
          Math.floor(((elapsed - 3000) / (totalDuration - 3000)) * 100),
          99
        );
        return {
          jobId,
  ...
  public estimateCost(request: VideoGenerationRequest): number {
    ...
    const baseCostPer15Seconds = 1.5;
