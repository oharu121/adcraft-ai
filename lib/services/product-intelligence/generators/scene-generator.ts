...
import { ProductCategory, KeyScenes } from "@/types/product-intelligence";
import type { CommercialStrategyTemplate } from "../constants/commercial-templates";
...
export class SceneGenerator {
  ...
  private buildFlexibleScenePrompt(
    ...
    const headline = typeof template.headline === "function" 
      ? template.headline(product) 
      : template.headline;
...
Effective patterns to consider: lifestyle integration, transformation stories, social proof, behind-the-scenes, problem/solution, aspiration, emotional connection, etc. Choose the approach that best fits this product type and target audience.`;
  ...
  private buildFlexibleScenesResponse(scenes: any[]): KeyScenes {
    return {
      ...
      productShowcase: scenes.find((s: any) => 
        s.purpose.includes("product") || s.purpose.includes("showcase")
      )?.description || scenes[1]?.description || "",
      problemSolution: scenes.find((s: any) => 
        s.purpose.includes("problem") || s.purpose.includes("solution")
      )?.description || scenes[2]?.description || "",
      emotionalMoment: scenes.find((s: any) => 
        s.purpose.includes("emotional") || s.purpose.includes("connection")
      )?.description || scenes[3]?.description || "",
