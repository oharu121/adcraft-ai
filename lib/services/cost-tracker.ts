...
export class CostTracker {
  ...
  private async checkRapidCostIncrease(): Promise<void> {
    try {
      ...
      const recentCosts = this.costHistory.filter(
      ...
      if (projectedDailySpend > this.totalBudget * 0.1) { // More than 10% of budget per day
        const alert: CostAlert = {
  ...
  public async getCostProjection(): Promise<CostProjection> {
    try {
      ...
      const projectedTimeToLimit = projectedHourlySpend > 0 
        ? remainingBudget / projectedHourlySpend 
        : Infinity;
  ...
  private async sendBudgetAlert(status: BudgetStatus): Promise<void> {
    ...
    if (!existingAlert || existingAlert.resolved) {
      const alert: CostAlert = {
        id: alertKey,
        type: 'threshold',
        severity: status.alertLevel === 'exceeded' ? 'critical' : 
                 status.alertLevel === 'danger' ? 'high' : 'medium',
        ...
        threshold: status.totalBudget * (status.alertLevel === 'warning' ? 0.75 : 0.90),
        timestamp: new Date(),
        resolved: false,
      };
