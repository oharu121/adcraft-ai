...
import { AnalysisResponse, ApiResponse } from "@/lib/agents/product-intelligence/types";
    ...
    const mockAnalysis = {
      ...
      commercialStrategy: {
        ...
        callToAction: {
          ...
          secondary:
            params.locale === "ja" ? ["詳細を見る", "比較する"] : ["Learn More", "Compare Models"],
        },
        storytelling: {
    ...
    return {
      ...
      suggestions:
        params.locale === "ja"
          ...
          : [
              "Please provide more details about your target audience",
              "Could you share information about the price range?",
            ],
    };
  }
}
...
async function updateSessionWithAnalysis(
  ...
  console.log(
    `Updating session ${sessionId} with analysis results, processing time: ${processingTime}ms`
  );
}
